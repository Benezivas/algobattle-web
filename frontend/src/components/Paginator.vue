...
<template>
  <nav v-if="total > PAGE_SIZE" aria-label="Table pagination">
    ...
    <li class="page-item">
      <a class="page-link" :class="{ disabled: currPage <= 1 }" @click="emit('update', (currPage - 1) * PAGE_SIZE)"
        ><i class="bi bi-chevron-compact-left"></i
          ></a>
        </li>
    <li class="page-item">currPage / maxPage</li>
    <li class="page-item">
      <a class="page-link" :class="{ disabled: currPage >= maxPage }" @click="emit('update', (currPage + 1) *  PAGE_SIZE)"
        ><i class="bi bi-chevron-compact-right"></i
          ></a>
        </li>
        <li class="page-item">
          <a class="page-link" :class="{ disabled: currPage >= maxPage }" @click="emit('update', maxPage * PAGE_SIZE)"
