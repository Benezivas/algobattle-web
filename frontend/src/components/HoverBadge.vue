...
<template>
  <span class="badge rounded-pill m-1" :class="bgClass">
    <slot />
    <a @click.prevent="e => emit('click')" href="#" @mouseover="e => hovering = true" @mouseleave="e => hovering = false"
      :title="props.type"><i class="bi" :class="[props.type == 'add' ? 'text-light bi-plus-lg': 'bi-x-lg']"></i></a>
  </span>
</template>
